...
def start_processing(path):
    ...
    model = YOLO("Model/rectangle.pt")  # путь к модели

    img = cv2.imread(path)
    lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
    lab_planes = list(cv2.split(lab))  # Преобразуем кортеж в список
    clahe = cv2.createCLAHE(clipLimit=8.2, tileGridSize=(8, 8))
    ...
    results = model(img, imgsz=2048, iou=0.001, conf=0.65, verbose=False, max_det=10000)
    ...
    if results[0].masks is not None:
        ...
        results_2048 = model(img, imgsz=1024, iou=0.04, conf=0.61, verbose=False, max_det=10000)
        results_704 = model(img, imgsz=640, iou=0.04, conf=0.61, verbose=False, max_det=10000)
        ...
        for contour in filtered_contours:
            ...
            if average_area - average_area/0.9 < mask_area < average_area + average_area/0.9:
    ...
    file_n = path.split(".")
    filename = f'{((file_n[0]).split("/")[-1])}.jpg'
    ...
    output_path2 = os.path.join(output_folder, filename)
    cv2.imwrite(output_path2, combined_image)


    return len(masks_list), output_path, output_path2
